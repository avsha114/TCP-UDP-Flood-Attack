...
bool validate_ip(const char *check)
{
    ...
    if (length < 7 || length > 15) //7-15 is the length range of an address as a string. For example - '8.8.8.8' ,'111.222.333.444'
...
{   
    ...
    int option = 0;
    bool udp_flood = false, manual_ip = false, manual_port = false;
    ...
    {        
        ...
        case 'r':
            udp_flood = true; // Enable UDP flood
            break;
    ...
    settings->protocol = udp_flood ? IPPROTO_UDP : IPPROTO_TCP;
